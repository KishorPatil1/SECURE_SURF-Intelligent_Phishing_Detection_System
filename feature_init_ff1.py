...
def has_dns_recording(domain):
    try:
        ...
        if creation_date is None:
            return -1  # or 0 depending on your usage
...
def number_of_emails(domain):
    try:
        ...
        number_emails = whois_info.get('emails')

        if not number_emails:
            return 0

        # If it's a string (single email), return 1
        if isinstance(number_emails, str):
            return 1

        # If it's a list or set, return its length
        if isinstance(number_emails, (list, set)):
...
def get_ssl_update_age(domain):
    try:
        ...
        if isinstance(cert_data, list):
            cert_dates = [date.astimezone(timezone.utc).replace(tzinfo=None) if date.tzinfo else date for date in cert_data]
            cert_date = max(cert_dates)
        else:
            cert_date = cert_data.astimezone(timezone.utc).replace(tzinfo=None) if cert_data.tzinfo else cert_data
        current_date_time = datetime.utcnow()
        age = current_date_time - cert_date
        return age.days
    except Exception as e:
        print(e)
        return -1
